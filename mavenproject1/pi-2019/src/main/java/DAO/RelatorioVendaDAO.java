...
public class RelatorioVendaDAO {
    public static List<Relatorio> listar()
     ...
     public static List<Relatorio> listarTodos()
        ...
        //Monta a string de listagem de clientes no banco, considerando
        //apenas a coluna de ativação de clientes ("enabled")
        String sql = "SELECT * FROM Venda join Carrinho ON Venda.IDVenda = Carrinho.IDCarrinho "
                + "join Livro ON Livro.ID = Carrinho.IDLivro join FormadePagamento ON\n" +
                " Venda.FormaPagamento = FormadePagamento.IDPagamento join Cliente ON Cliente.ID "
                + "= Venda.IDCliente ORDER BY IDVenda";
        //Lista de clientes de resultado
        List<Relatorio> listaRelatorio = null;
        //Conexão para abertura e fechamento
        Connection connection = null;
        ...
        try {
            ...
            while (result.next()) {
                ...
                String cliente = result.getString("Cliente.Nome");
                String data = result.getString("Venda.DataVenda");
                float valor = result.getFloat("Venda.Valor");
                String formaPagamento = result.getString("FormadePagamento.Descricao");
                
                

                Relatorio r = new Relatorio(cliente, IDVenda, data, valor, formaPagamento);
                //Adiciona a instância na lista
                listaRelatorio.add(r);
            }
        } finally {
